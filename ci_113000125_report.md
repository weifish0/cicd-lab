...
#TODO：請貼上 GitHub Actions 成功執行的 workflow summary 截圖。

### GitHub Actions 結果頁面

#TODO：請貼上 GitHub Actions 中顯示 Vitest test results 的結果頁面截圖。

## 失敗案例說明

可使用「測試失敗」作為故意製造錯誤的案例。例如暫時將 `test/app.test.ts` 中首頁訊息的期望值改成錯誤文字：
...
#TODO：請貼上 GitHub Actions pipeline failed 的截圖，需包含失敗的 Test step 或失敗的 test case。

### 失敗原因與修正方式截圖

#TODO：請貼上 GitHub Actions 中 test failure 詳細錯誤訊息的截圖，並標示修正後重新執行成功的結果。
